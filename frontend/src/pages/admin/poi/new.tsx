...
const NewPoi = () => {
	...
	const validateImages = (images: string[]) => {
		if (images.length < 5) {
			toast.error("Cinq images sont requises.");
			return false;
		}
		...
		if (invalidImage) {
			toast.error(
				"Les images téléchargées ne sont pas au bon format (les formats d'images supportés sont les suivants : JPEG, JPG ou PNG)."
			);
			return false;
		}
	...
	const onSubmit: SubmitHandler<POIInput> = async (formData: POIInput) => {
		...
		await createNewPoi({
			...
			.then((res) => {
				...
				setImageURLs([]);
				reset();
				router.push(`/poi/${res.data?.createNewPoi?.id}`);
			})
			.catch((err) => {
				toast.error(
