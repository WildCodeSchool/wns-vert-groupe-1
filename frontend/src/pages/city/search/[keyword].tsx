		...
		<>
			<Head>
				<title>
					...
					{searchedCity?.name
						? `de la ville de ${capitalizeFrenchName(searchedCity.name)}`
						: ""}{" "}
					| CityGuide
				</title>
				<meta
			...
			<Box display="flex" width="100%" height={window.innerHeight - 120}>
				<Box flex="1" height="100%" overflow="auto">
					<Box padding={mainTheme.spacing(6)}>
						<SearchForm />
					</Box>

					{searchedCity && searchedCity.name !== "" && data ? (
						<>
							<Box
								display="flex"
								alignItems="center"
							...
							<Box padding={mainTheme.spacing(6)}>
								...
								>
									{searchedCity?.name
										? capitalizeFrenchName(searchedCity.name)
										: ""}{" "}
									({filteredPois.length})
								</Typography>
				...
				{dataFetched && (
					<Grid item flex="1" width="100%" display="grid">
						<CityMap
					...
					{searchedCity.name !== "" ? (
						...
						>
							...
							>
								...
								{categoryTags.map((category: CategoryType) => (
									<MenuItem
										key={category.id}
										value={category.name}
										color={mainTheme.palette.primary.main}
